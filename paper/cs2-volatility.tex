...
The paper appears to be the first out-of-sample evaluation of volatility
forecasts in a virtual asset market to test them against benchmarks.
...
The closest antecedent is an unpublished course essay. \citet{Nikolaenko2025}
fits ARMA--GARCH(1,1) specifications to two CS:GO rifle skins using 826 daily
observations from the Buff.163 marketplace over July 2021 to February 2024,
applies Bai--Perron tests for structural breaks in log prices, and computes
expanding-window one-step-ahead conditional variance forecasts over a 200-day
evaluation sample, reporting RMSE, MAE and QLIKE and concluding that predictive
power is modest.

We differ in three respects. First, scope: twenty-two items stratified across
four price tiers, drawn from the Steam Community Market, against two mid-tier
rifle skins from a secondary venue. Second, and centrally, \emph{comparison}.
Reporting a QLIKE value establishes that a model produces forecasts; it does
not establish whether those forecasts are useful, because a loss statistic has
no interpretation in isolation. Our forecasts are scored against parameter-free
benchmarks and tested with \citet{DieboldMariano1995} and \citet{HLN2011}, so
that ``modest predictive power'' becomes a comparative claim with a sign and a
significance level. Third, breaks: \citet{Nikolaenko2025} treats structural
breaks as a nuisance motivating the use of returns over price levels, whereas
we use a single dated policy change as an identified shock and ask how
forecasts behave across it. His sample ends in February 2024 and therefore
pre-dates that episode entirely. We note that his concluding section names the
construction of a daily panel of liquid skins as the natural next step.

One apparent disagreement should be recorded. \citet{Nikolaenko2025} finds
little serial dependence in returns---BIC selects a white-noise mean for one of
his two series---whereas we reject the null of no autocorrelation on every item
in our panel (Section~\ref{sec:facts}). The comparison is not direct: the two
studies use different marketplaces, currencies, wear grades and sample periods,
and two series cannot detect a weekly seasonal that is individually significant
on fewer than half of ours. We regard the results as complementary rather than
contradictory, but the discrepancy is a natural target for replication.
...
\newblock Course essay, Time Series and Panel Data Analysis, International
College of Economics and Finance, HSE University, December 2025. Unpublished;
