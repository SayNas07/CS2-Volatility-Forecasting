...
three facts. First, returns exhibit genuine ARCH effects (22 of 24 items) but
...
\noindent
Our contribution is threefold. Section~\ref{sec:facts} documents the
...
equities. Section~\ref{sec:models} estimates AR(1)-GARCH, EGARCH and
GJR-GARCH specifications with Student-$t$ errors and shows that volatility
persistence does not vary systematically with item value---a negative result
that contradicts a gradient visible in a naive proxy.
...
Our paper is distinct from all of these in object. The existing work studies
\emph{returns}---their level, their cross-section, their risk exposures---or
\emph{liquidity} around a single event. We study the \emph{forecastability of
conditional variance}, which to our knowledge has not been examined in any
virtual asset market. The question of whether standard volatility technology
transfers to a market without professional intermediation is separate from,
and not answered by, the finding that skins earn high average returns.
...
specification is misspecified, and motivates the AR(1) mean used throughout
...
The ordering is, if anything, the reverse of the hypothesis. We regard this as
a methodological result rather than an economic one: absolute-return
